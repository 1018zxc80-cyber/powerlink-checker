    ...
    try:
        ...
        for product_name, items in products.items():
            ...
            for r in items:
                ...
                message += f"{r['keyword']}  {pc_text} / {mobile_text}\n"
...
def add_product():
    ...
    try:
        ...
        if not product or not keyword or not search_name:
            return jsonify({"status": "error", "message": "모든 필드를 입력하세요"}), 400
        ...
        try:
            ...
            product_sheet = spreadsheet.add_worksheet(title="제품", rows=1000, cols=10)
            product_sheet.append_row(["제품명", "키워드", "찾을이름"])
        
        product_sheet.append_row([product, keyword, search_name])
        
        return jsonify({"status": "success", "message": "제품이 추가되었습니다"})
...
def delete_product(row_id):
    ...
    try:
        ...
        product_sheet.delete_rows(row_id)
        
        return jsonify({"status": "success", "message": "제품이 삭제되었습니다"})
...
def update_product(row_id):
    ...
    try:
        ...
        product_sheet.update(f'A{row_id}:C{row_id}', [[product, keyword, search_name]])
        
        return jsonify({"status": "success", "message": "제품이 수정되었습니다"})
