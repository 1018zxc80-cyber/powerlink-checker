...
def get_advertisers_from_sheet():
    """구글 시트의 '광고주' 시트에서 광고주+블로그ID 읽어오기"""
    try:
        ...
        try:
            ad_sheet = spreadsheet.worksheet("광고주")
        except:
            print("'광고주' 시트를 찾을 수 없습니다.")
            return [], []
        
        all_data = ad_sheet.get_all_values()
        
        if not all_data:
            return [], []
        
        # 헤더 제외
        if all_data[0][0] in ["광고주", "advertiser", "Advertiser"]:
            all_data = all_data[1:]
        ...
        for row in all_data:
            if len(row) >= 1 and row[0].strip():
                advertisers.append(row[0].strip())
            if len(row) >= 2 and row[1].strip():
                blog_ids.append(row[1].strip())
        
        return advertisers, blog_ids
...
def check_powerlink(driver, keyword, advertisers, blog_ids):
    """파워링크 노출 확인"""
    result = {
        "keyword": keyword,
        "found": False,
...
def save_to_google_sheet(results, advertisers, blog_ids):
    """구글 시트의 '결과' 시트에 저장"""
    ...
    try:
        ...
        if not existing:
            result_sheet.append_row(["번호", "키워드", "노출여부", "순위", "매칭", "광고주목록", "블로그목록", "확인시간"])
            next_num = 1
        else:
            try:
...
def check():
    ...
    try:
        # 광고주 목록 읽기
        advertisers, blog_ids = get_advertisers_from_sheet()
        
        if not advertisers and not blog_ids:
            return jsonify({
                "status": "error", 
                "message": "광고주가 없습니다. 구글 시트의 '광고주' 시트를 확인하세요."
            }), 400
        ...
        for keyword in keywords:
            result = check_powerlink(driver, keyword, advertisers, blog_ids)
            results.append(result)
            time.sleep(1.5)
        
        save_to_google_sheet(results, advertisers, blog_ids)
