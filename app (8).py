...
def check_powerlink_single(driver, keyword, search_name):
    """단일 키워드 파워링크 노출 확인"""
    result = {
        "found": False,
        "position": None
...
def check_powerlink(pc_driver, mobile_driver, product_data):
    ...
    # PC 체크
    pc_result = check_powerlink_single(pc_driver, keyword, search_name)
    
    # 모바일 체크
    mobile_result = check_powerlink_single(mobile_driver, keyword, search_name)
        ...
        for product_name, items in products.items():
            ...
            for r in items:
                ...
                if r["pc_found"] and r["pc_position"]:
                    pc_text = f"PC {r['pc_position']}위 ✅"
                else:
                    pc_text = "PC 미노출 ❌"
                
                # 모바일 순위
                if r["mobile_found"] and r["mobile_position"]:
                    mobile_text = f"모바일 {r['mobile_position']}위 ✅"
                else:
                    mobile_text = "모바일 미노출 ❌"
